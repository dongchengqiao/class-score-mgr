...
│   ├── student.json          # 学生（students，含各自 history）
...
- 数据按实体拆分存储于项目根目录 `data/` 下的多个 JSON 文件，**无数据库依赖**，直接可读可备份：
  - `config.json` — 设置 + 自增序列
  - `student.json` — 学生
  - `group.json` — 小组
  - `rule.json` — 规则
...
- 备份数据只需拷贝整个 `data/` 目录（或其中的 `config.json` / `student.json` / `group.json` / `rule.json` / `shouItems.json`）